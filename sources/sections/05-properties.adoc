...
Some of the possible values for the KIND property are "Location", "Individual", "Device", or
"Group".
...
Device is used for a computing device such as an appliance, a computer, or a network element.

Group is used to specify a group of resources with a specific skill set. For example: drivers,
electricians, etc.

[[existing_attrs]]
=== Base vCard Properties

The following properties defined in <<RFC6350>> or <<RFC2739>> make sense for vCards representing
resources (this list is not exhaustive, and other properties might be applicable as well):
...
Purpose:: Specify special resource accessibility info for the physically disabled people.
...
ValueType:: Value MAY be a URI that could be a vCard. The vCard could be of the KIND group
whose members point to the various inventory items.
+
Value MAY also be a free form text listing
...
Purpose:: Specify if there are restrictions to physically accessing the resource. For example
...
Purpose:: Specify extra information on physically accessing resources that have restrictions.
For example where to pick up keys to a locked resource.
ValueType:: URI value. It MAY also be a free-form text value.
Cardinality:: *
ABNF::
+
...
----
Special Notes:: If this property is absent, it indicates that the resource may be booked free of cost.
Default value:: TRUE
Example value:: TRUE
...
Purpose:: Provide the URL pointing to complete pricing information for usage of the resource.
ValueType:: URI value. It MAY also be a free-form text value.
Cardinality:: *
ABNF::
+
